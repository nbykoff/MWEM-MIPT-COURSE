...
u1 = u(N, nt, 1)
u2 = u(N * k, nt * k ** 3, 9)
u3 = u(N * k ** 2, nt * k ** 6, 81)

c_norma = []
for n in range(nt):
    norm1 = norm_C(u3[5::k ** 2, n * k ** 6] - u2[2::k, n * k ** 3])
    norm2 = norm_C(u2[2::k, n * k ** 3] - u1[1:N + 1, n])
    c_norma.append(norm2 / norm1)

l_norma = []
for n in range(nt):
    norm1 = norm_L2(u3[5::k ** 2, n * k ** 6] - u2[2::k, n * k ** 3])
    norm2 = norm_L2(u2[2::k, n * k ** 3] - u1[1:N + 1, n])
    if norm1 != 0:
...
plt.plot(time, [81*9] * nt, label='Ref=729')
plt.grid()
plt.legend()
plt.show()
